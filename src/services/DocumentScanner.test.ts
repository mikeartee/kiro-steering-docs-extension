...
suite('DocumentScanner Test Suite', () => {
  ...
  setup(() => {
    ...
    // Use the actual kiro-steering-docs repository path (if it exists)
    repoRoot = path.join(__dirname, '../../../kiro-steering-docs');
    repoExists = fs.existsSync(repoRoot);
  });

  test('scanRepository should find documents in target directories', async function() {
    if (!repoExists) {
      this.skip(); // Skip in CI where the repo doesn't exist
      return;
    }
  ...
  test('shouldInclude should exclude documentation files', async function() {
    if (!repoExists) {
      this.skip();
      return;
    }
  ...
  test('shouldInclude should exclude docs directory', async function() {
    if (!repoExists) {
      this.skip();
      return;
    }
  ...
  test('shouldInclude should exclude templates directory', async function() {
    if (!repoExists) {
      this.skip();
      return;
    }
  ...
  test('determineSubcategory should detect subcategories from path', async function() {
    if (!repoExists) {
      this.skip();
      return;
    }
  ...
  test('scanRepository should only include markdown files', async function() {
    if (!repoExists) {
      this.skip();
      return;
    }
